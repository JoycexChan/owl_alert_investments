...
function calculateMovingAverage(data: number[], period: number, dates: string[]): { date: string; average: number }[] {
  let averages: { date: string; average: number }[] = [];
  for (let i = period - 1; i < data.length; i++) {
    let sum = 0;
    for (let j = i - period + 1; j <= i; j++) {
      sum += data[j];
    }
    averages.push({ date: dates[i], average: sum / period });
  }
  return averages;
}
...
const EnhancedKLineChart: React.FC<{ stockCode: string }> = ({ stockCode }) => {
  ...
  const fiveDayMA = calculateMovingAverage(closePrices, 5, dates);
  const twoWeekMA = calculateMovingAverage(closePrices, 10, dates);
  const monthMA = calculateMovingAverage(closePrices, 20, dates);
