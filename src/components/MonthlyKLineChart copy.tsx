...
const EnhancedKLineChart: React.FC<{ stockCode: string }> = ({ stockCode }) => {
  ...
  const fiveDayMADates = dates.slice(4); // 切掉前 4 個日期，與 5 日移動平均數據對應
  const twoWeekMADates = dates.slice(9); // 切掉前 9 個日期，與 10 日移動平均數據對應
  const monthMADates = dates.slice(19); // 切掉前 19 個日期，與 20 日移動平均數據對應
